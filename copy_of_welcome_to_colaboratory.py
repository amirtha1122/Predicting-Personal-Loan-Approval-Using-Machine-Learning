...
Original file is located at
    https://colab.research.google.com/drive/1UXULomYl8MJ2z0MYj-YCDXzmKsSMWqWs
...
data["Gender"].fillna(data["Gender"]_node{}[0])

||\data['Married" } = data["Married"]_F11Ira(data['Harried"]-mode()[0]}

data["dependents"]-datis{/ompendek>"}.str.replace("»^,^^)

data['Dependents"]

= data['Oependents"]-filine[data['Dependents"]-mode{}{9])\\

data['Self_Impluyca'] = data['$=1#_Employed"},{11}nnidata['self_Employed'}.mode()[0])

data["Credit_History"] = data['Credit

History").filla/datal 'Credit

History').mode()[0])

data[ "Gender"]-data['Gender'].astype("int64") data[ "Married"]=data['Married'].astype("Int64")

data[ "Dependents'] data['Dependents'].astype('int64") data[ 'Self Employed' ]=data['Self Employed'].astype("int64') data['Coapplicant Income']=data['Coapplicant Income'].astype("Int64') data['LoanAmount']-data['LoanAmount"].astype("int64")

data["Loan_Amount_Term"]=data['Loan_Amount_Term"].astype("int64") data['Credit_History"]-data['Credit History'].astype('int64")
...
smote- SHOTETomek (0.90)

5)Marx\P\AppData\Python\Python\site-packages\comJammiert£Ition.py:7: Future Pass samling strategy.

sfividing the sitejet inte appercent and Endependently and respecting y catal Loan_Status"]

x-cata, drep(columns>["Loan_status"],axis=1)

#orvetting a new a me vertibles for the belived det

sortestop the values of before butanedite and after

print(y.value_counts()) print(y_bal.value_counts())
...
plt.subplot(121)
sns.distplot(data['applicationincome'], color-'r')
plt.subplot(122)
...
warnings.warn( C:\Users\HP\anaconda\lib\site-packages\seaborn\_decorators.py:36: Futurewarning: Pass the foll ersion 0.12, the only valid positional argument will be data, and passing other arguments witho error or misinterpretation.
...
sns.swarplot(data['gender'],data['applicantincome'], hue = data['loan_status'])
...
model_history = classifier.fit(x_train, batch_size=100,validation_split=0.2, epochs=100)
...
sample value = [[1,1, 8, 1, 1, 4276, 1542, 145, 240, 0,1]]

if predict_exit(sample value)>0.5: print("Prediction: High chance of Loan Approval!")

else:

print("Prediction: Low chance Loan Approval.")

sample value [[1,0, 1, 1, 1, 45, 14,45, 240, 1,1]] if predict_exit(sample_value)>0.5: print("Prediction: High chance of Loan Approval)
...
def compareModel(x_train,x_test,y_train,y_test):
   ...
   print('_'*100) 
   XGB(X_train,Xx_test,y_train,y_test) 
   print('_'*100)
...
comparemode1(x_train,y_train,y_test)

classifier.predict(x_test)

ypred

print(accuracy_score (y_pred,y_test)) print("ANN Model")
...
pickle.dump(mode1,open('rdf.pk1','wb'))
...
model = pickle.load(open(r'rdf.pkl', 'rb')) scale = pickle.load(open(r'scale1.pkl', 'rb'))
...
Bopp.route("/sutwit methods-["POST", "GET"}}# route to show the predictions in web UI

der submit():

reading the Inputs given by the user

input_feature=[int(x) for x in request.form.values() 1 #input feature no transpose(input feature)

input feature (np.array(input feature)] print(input feature) names - I 'Gender, 'Harried, "Dependents, Education", "Self Employed", "Applicant income", Coopplicant Income, LoanAmount", "Loon Amount Term', 'Credit History', 'Property Area 3 data pandas.DataFrame(input feature,columns-names) print(data)

scata scaled scale.fit transform(data) sdata pardes.Dataframe.columns=names)

#predictions using the loaded model file

prediction-model.predict(date) print(prediction)
...
return render_template("output.htet" result="Loun wilt Not be Approved") else: return render_template("output.html",result = "Loan will be Approved")

#showing the prediction results in a 1
...
Environment: production WARNING: This is a development server. Do not use it in a r
